...
static const char* const CLASS = "CatoOver";

static const char* const HELP =
  "Puts one image over the other";
 ...
 */
static Iop* CatoOverCreate(Node* node)
{
  return new CatoOver(node);
}
 ...
 */
const Iop::Description CatoOver::description ( CLASS, "Merge/CatoOver",
                                                     CatoOverCreate );


void CatoOver::_validate(bool for_real)
{
  copy_info(); // copy bbox channels etc from input0
  merge_info(1); // merge info from input 1
}

void CatoOver::_request(int x, int y, int r, int t, ChannelMask channels, int count)
{
 ...
 */
void CatoOver::engine ( int y, int x, int r,
                              ChannelMask channels, Row& row )
{
  ...
  foreach ( z, channels ) {
    ...
    const float* alpha1 = input1Row[Chan_Alpha] + x;
